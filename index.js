...
servidor.get("/mostrarmascotas", function (request, response) {
    db.collection("mascotas").find().toArray(function (error, datos) {
        if (error !== null) {
            response.send(error);
...
//Borrar ficha mascota//

servidor.delete("/borrarMascota/:nombre", function (request, response) {
    const nombre = request.params.nombre;
    ...
    db.collection("mascotas").find({ nombre: nombre }).toArray(function (error, mascota) {
        if (error !== null) {
            ...
            if (mascota.length === 0) {
                ...
                db.collection("mascotas").deleteOne({ mascota: mascota }, function (error, datos) {
                    if (error !== null) {
                        response.send(error);
                    } else {
                        response.send({ mensaje: "¡Ficha borrada!" })
                    }
                })
            }
        }
